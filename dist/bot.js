...
function parsePreview(previewRaw) {
...
function bold(txt) { return fmt.bold(txt); }
function code(txt) { return fmt.code(txt); }
// --- ganti fungsi ini saja ---
export const sendAdminCard = async (sessionId, preview) => {
    const site = process.env.SITE_NAME || 'McX Live Chat';
    // 1) Parse dari preview (untuk prechat)
    const { isPrechat, username: uFromPreview, pertanyaan, message } = (function parse(previewRaw) {
        const preview = String(previewRaw || '');
...
export const sendAdminImage = async (sessionId, data) => {
    try {
        ...
        if (m) {
            ...
            if (canAsPhoto) {
                await bot.telegram.sendPhoto(ADMIN_CHAT_ID, { source: buf, filename: `photo_${sessionId}.${ext}` }, { caption: `Dari ${sessionId}` });
            }
            else {
                await bot.telegram.sendDocument(ADMIN_CHAT_ID, { source: buf, filename: `image_${sessionId}.${ext}` }, { caption: `Dari ${sessionId}` });
            }
            return;
        }
        // treat as URL
...
async function tgFileToDataUrl(fileId, mimeFallback = 'image/jpeg') {
...
/* ----------------- commands ----------------- */
bot.command('start', (ctx) => ctx.reply('Operator ready. /ls, /use <sid>, /r <sid> <message>.'));
// List chat aktif (tulisan & tombol pilih)
bot.command('ls', async (ctx) => {
    const sessions = listOpenSessions(50); // ambil lebih banyak
    if (!sessions.length)
        return ctx.reply('Tidak ada chat aktif.');
    const lines = sessions.map((s, i) => `${i + 1}. ${code(s.id)} — ${s.visitor_name || '-'}`).join('\n');
...
bot.command('use', async (ctx) => {
    ...
    const sid = parts[1];
    if (!sid)
        return ctx.reply('Usage: /use <sessionId>');
    activeSessionId = sid;
    return ctx.reply(`Active session: ${sid}`);
});
/* --------- kirim teks admin → user --------- */
export const routeAdminMessage = async (sessionId, text) => {
...
bot.command('r', async (ctx) => {
    ...
    if (!match)
        return ctx.reply('Usage: /r <sessionId> <message>');
...
bot.on('callback_query', async (ctx) => {
  ...
  try {
    // auto-ack cepat supaya tidak timeout
    ...
    if (data.startsWith('use:')) {
      ...
      await ctx.reply(`Active session: ${activeSessionId}`);
      return;
    }
    ...
    if (err.description?.includes('query is too old')) {
      console.warn('Callback query kadaluarsa, diabaikan.');
    } else {
...
bot.on('text', async (ctx) => {
    ...
    if (!text)
        return;
    if (!activeSessionId)
        return ctx.reply('No active session. /use <sessionId> dulu.');
...
bot.on('photo', async (ctx) => {
    ...
    try {
        ...
        if (ctx.message.caption)
            await routeAdminMessage(activeSessionId, ctx.message.caption);
    }
    catch (err) {
...
bot.on('document', async (ctx) => {
    ...
    if (!mime.startsWith('image/'))
        return ctx.reply('Dokumen ini bukan gambar. Kirim foto/gambar saja 👍');
    try {
...
let ioEmit = null;
export const bindEmitter = (fn) => { ioEmit = fn; };
